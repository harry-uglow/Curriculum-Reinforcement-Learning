...
class SawyerEnv(VrepEnv):
    num_joints = 7
    action_space = spaces.Box(np.array([-0.02]*3 + [-0.5]*3), np.array([0.02]*3 + [0.5]*3),
                              dtype=np.float32)
    target_point = np.array([0.] * 6)
    scale = 0.01
    ...
    def reset(self):
        ...
        self.target_point = np.array([0.] * 6)
    ...
    def update_sim(self):
        self.call_lua_function('update_robot_movement', floats=self.target_point)
